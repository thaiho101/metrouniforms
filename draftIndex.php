			...
			<div class='noticeModalContentAlignment'>
				...
				<div class='closeNoticeModalDiv'>
					<form action="" method="post">
						<button type='submit' name='closeNoticeModal' class="button okayNoticeButton">Close</button>
					</form>
				</div>
				...
				<div id="dataSecurityInfo">
					<div class='noticeContent'>
						...
						<div>This page is part of the Metro Uniforms management platform designed to track and manage employee voucher status for UT Southwestern Organization, 
						...
						<div class='noticeFooter' style='margin-top: 10%;'>Designed and Developed by Nam Ho</div>
    ...
    <div class='filterDetails'>
        <div class='centerInputRow'>
            ...
            <select name="companyName" id='companyName' class='borderRadius6 filterInputStyle'>
                ...
                <?php
                    $sql = "SELECT DISTINCT company FROM group_benefits ORDER BY company ASC";
                    $statement = $conn->prepare($sql);
                    $statement->execute();
                    $result = $statement->get_result();
                    
                    // Retain selected company in the dropdown
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $selected = (isset($_GET['companyName']) && $row['company'] === $_GET['companyName']) ? 'selected' : '';
...
<script>
// Filter function of Ajax on script file
</script>
				...
				echo "<table border='.2' class='policyTableStyle borderRadius6'>
					<thead>
						...
						<tr>
							...
							<td class='dataTableSize blueText rightCell policyContentCoverDiv'><div class='whiteCell'>$" . $voucherPolicy . "</div></td>
						</tr>
						<tr>
							<td class='boldText subTitleCellBackG'>Discount</td>
							<td class='dataTableSize blueText rightCell policyContentCoverDiv'><div class='whiteCell'>%" . $discountPolicy . "</div></td>
						</tr>
						<tr>
...
<dialog id="myDialog">
<div class='dialogCloseButton'><button onclick="document.getElementById('myDialog').close()" class='button'>Close</button></div>
